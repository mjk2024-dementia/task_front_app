...
const Item = ({ task }) => {
  ...
  const deleteItem = async() => {
    const confrim = window.confirm('아이템을 삭제하시겠습니까?');
    // console.log(confrim);
  ...
  const changeCompleted = async() => {
    ...
    // console.log(updateCompletedData); 
  ...
  return (
    <div className="item w-1/3 h-[25vh] p-[0.25rem]">
      <div className="w-full h-full border border-gray-500 rounded-md flex py-3 px-4 flex-col justify-between bg-gray-950">
        <div className="upper">
          <h2 className="text-base font-normal mb-3 relative pb-2 flex justify-between">
            <span className="w-full h-[1px] bg-gray-500 absolute bottom-0"></span>
            {title}
            <span className='text-xs py-1 px-3 border border-gray-500 rounded-md hover:bg-gray-700 cursor-pointer'>자세히</span>
          </h2>
          <p style={{ whiteSpace: 'pre-wrapp' }}>{description}</p>
        </div>
        <div className="lower">
          ...
          <div className="item-footer flex justify-between">
            <div className="item-footer-left flex gap-x-2">
              {iscompleted ? (
                <button 
                className="block py-1 px-4 bg-green-400 text-sm text-white rounded-md" onClick={changeCompleted}
                >
                  Completed
                </button>
              ) : (
                <button className="block py-1 px-4 bg-cyan-500 text-sm text-white rounded-md" onClick={changeCompleted}
                >
                  Incompleted
                </button>
              )}
